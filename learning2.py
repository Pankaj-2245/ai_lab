...
from pygame.locals import QUIT, KEYDOWN, K_ESCAPE, K_RETURN, K_LEFT, K_RIGHT, K_UP, \
    K_DOWN  # Import common Pygame constants
...
import json  # For saving and loading the Q-table in a human-readable format
import os  # For file operations
import ast  # For safely evaluating string representations of tuples when loading JSON
...
class QLearningAgent:
    ...
    def get_action(self, state):
        ...
        if random.random() < self.epsilon:
            ...
            # Convert state tuple to string for Q-table lookup
            state_key = str(state)
            if state_key not in self.q_table:
                self.q_table[state_key] = [0.0] * self.n_actions  # Initialize if new state
            ...
            for i in valid_actions_indices:
                q_values_for_valid_actions[i] = self.q_table[state_key][i]
    ...
    def get_reward(self, old_state, action, new_state, game_over, ate_apple):
        ...
        if game_over:
            ...
            # To get new_head_x, new_head_y we need to simulate the move based on the action
            # This is a bit redundant with _get_potential_head but necessary here for reward calculation
            current_head_x, current_head_y = self.game.snake.x[0], self.game.snake.y[0]
            simulated_next_head_x, simulated_next_head_y = current_head_x, current_head_y
            action_name = self.actions[action]

            if action_name == 'left':
                simulated_next_head_x -= SIZE
            elif action_name == 'right':
                simulated_next_head_x += SIZE
            elif action_name == 'up':
                simulated_next_head_y -= SIZE
            elif action_name == 'down':
                simulated_next_head_y += SIZE

            new_dist_sq = (simulated_next_head_x - self.game.apple.x) ** 2 + (
                    simulated_next_head_y - self.game.apple.y) ** 2
    ...
    def update_q_table(self, state, action, reward, next_state, done):
        ...
        """
        # Convert state tuples to strings for JSON keys
        state_key = str(state)
        ...
        if done:
            ...
            if next_state_valid_actions:
                max_future_q = max([self.q_table[next_state_key][i] for i in next_state_valid_actions])
    ...
    def get_valid_actions_for_state(self, state_tuple):
        ...
        if not valid_actions:
            return list(range(self.n_actions))  # Allow all actions as a last resort to prevent infinite loop
        ...
        while True:
            ...
            try:
                ...
                if self.game.snake.length > old_length:
                    ate_apple = True
                    # Apple is already moved by game.play() based on training_mode
                    # but for training specifically, we enforce random move here.
                    self.game.apple.move_random()
        ...
        if os.path.exists(filename):
            try:
                with open(filename, 'r') as f:
                    ...
                    self.q_table = {ast.literal_eval(k): v for k, v in loaded_q_table_str_keys.items()}
                print(f"Q-table loaded from {filename}. Size: {len(self.q_table)} entries.")
            except json.JSONDecodeError:
                print(f"Error decoding JSON from {filename}. Starting with an empty Q-table.")
                self.q_table = {}
            except ValueError as e:
                print(f"Error evaluating Q-table keys from {filename}: {e}. Starting with an empty Q-table.")
                self.q_table = {}
        else:
    ...
    def play_game(self):
        ...
        # Set training_mode to False so score and time are displayed
        self.game.training_mode = False
        # Set agent_is_playing flag to true
        self.game.agent_is_playing = True
    ...
    def save_q_table(self):
        ...
        Keys (states) are converted to strings.
        """
        try:
            # Convert tuple keys to string keys for JSON serialization
            serializable_q_table = {str(k): v for k, v in self.q_table.items()}
            with open(self.q_table_file, 'w') as f:
                json.dump(serializable_q_table, f, indent=4)  # Use indent for readability
...
class Game:
    ...
    def __init__(self):
        ...
        self.agent_is_playing = False  # New flag to indicate if agent is playing
    ...
    def play(self):
        ...
        # Display score and time always, unless in training_mode (which is only for actual training, not agent playing)
        ...
        if self.is_collision(self.snake.x[0], self.snake.y[0], self.apple.x, self.apple.y):
            ...
            if not self.training_mode:  # Only play sound if not in training mode
            ...
            # Ensure game_speed does not go below a small positive value (e.g., 0.05)
            if (self.snake.length) % 5 == 0:
                self.game_speed = max(0.05, self.game_speed - 0.04)  # Ensure it's never negative or too low

            # Apple movement logic: random for training/agent play, predefined for manual play
            if self.training_mode or self.agent_is_playing:  # If training or agent playing, use random apple
                self.apple.move_random()
            else:  # For manual human play, use predefined apple sequence
                self.apple.move()
    ...
    def show_game_over(self, message="Game Over!"):
        ...
        """
        # Removed the `if self.training_mode:` check here.
        # The game over screen should always appear when a game ends, regardless of who was playing.
        ...
        while running:
            ...
            for event in pygame.event.get():
                if event.type == KEYDOWN:  # A key has been pressed.
                    ...
                    # Training controls
                    if event.key == pygame.K_t and not pause:  # T key to train agent
                    ...
                    if event.key == pygame.K_a and not pause:  # A key to activate agent
                        agent_playing = True
                        self.reset()
            ...
            if agent_playing and not pause:
                try:
                    self.agent.play_game()  # This will now display score/time and show game over
                    # Agent finished playing (game over)
                    agent_playing = False
                    pause = True
                except Exception as e:
                    # Game over for agent
                    agent_playing = False
                    pause = True
                    self.show_game_over(str(e))  # Display game over for agent too

            # Game logic and rendering executed only if the game is not paused.
            try:
                if not pause and not agent_playing:
            ...
            # This sleep is applied to both manual and agent play modes, but not training mode
    ...
    def show_instructions(self):
        ...
        print("4. The Q-table is automatically saved ('q_table.json') and loaded upon startup.")
        print("5. To reset the agent's learning, simply delete the 'q_table.json' file.")
        print("=" * 60)
